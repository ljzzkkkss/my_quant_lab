...
- **板块共振过滤** - 行业 ETF 联动分析
- **量比/RSI/斜率** - 多维技术面过滤
- **千股千策** - 支持为不同股票配置专属策略和参数（`configs/my_routing_dict.json`）
...
### 标签页 6：⚡ 盘中实时狙击 (Auto-Funnel Sniper)

**适用场景**: 盘中实时捕捉全市场活跃标的买点

**核心原理**: 通过**【漏斗初筛】+【数据缝合】+【策略推演】**三层过滤机制

1. **🌪️ 第一层：全市场漏斗初筛** - 过滤 95% 垃圾股
   - 设置股价范围（最低/最高股价）
   - 设置最低成交额（过滤死水股）
   - 设置最高扫描数量（防止拉取超时）
   - 强制过滤 ST 股与退市股

2. **⚔️ 第二层：主力狙击策略指派** - 选择用于判定买点的策略模型

3. **🧵 第三层：数据缝合与策略推演** - 对每只股票：
   - 拉取历史 K 线数据
   - 缝合当天实时快照（今开、最高、最低、最新价）
   - 执行策略信号生成
   - 应用高级过滤器（大盘/板块/ML 拦截）
   - 判断是否产生实时买点
...
**特色功能**: 支持**千股千策** - 如果某股票在 `configs/my_routing_dict.json` 中有专属配置，会自动使用该股票专属的策略和参数进行推演

**最佳使用时间**: 交易日 14:50 左右（尾盘集合竞价前），捕捉当日买点
...
| 按钮 | 功能 |
|------|------|
| 💾 保存当前配置 | 永久保存所有参数到本地工作区 |
| 🔄 恢复默认值 | 清空所有设置，重启应用 |
| 🔄 强制更新股票代码库 | 清除缓存，重新拉取全市场 5000+ A 股与 ETF |
...
在 `configs/my_routing_dict.json` 中可以为不同股票配置专属策略和参数：
...
当使用**盘中实时狙击**功能时，系统会自动读取该配置，为不同股票使用专属策略和参数进行推演。
