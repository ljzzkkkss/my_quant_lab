...
- 📡 **雷达全场扫描** - 批量策略回测与牛股筛选
- 🧺 **组合轮动实战** - 多股轮动 + 动态资金分配
- 🏟️ **策略角斗场** - 多策略同标的无情厮杀，寻找"天命策略"

系统采用 **AkShare + Baostock** 双引擎数据源，支持 **8 种经典量化策略**，配备机构级回测引擎和可视化分析工具。
...
- **硬性止盈止损** - 固定比例触发（默认止盈 15%，止损 -8%）
- **动态追踪止损** - 盈利达标后自动启用回撤保护（盈利 10% 激活，回撤 5% 平仓）
- **大盘择时过滤** - 沪深 300ETF 均线择时
- **板块共振过滤** - 行业 ETF 联动分析
- **量比/RSI/斜率** - 多维技术面过滤
...
- **宏观情绪探针** - 黄金 ETF(518880)、标普 500、国债 ETF
- **地缘恐慌探针** - 原油 ETF(512710)、军工 ETF
...
│   └── advanced_filter.py # 高级过滤器（含 ML 引擎）
├── backtest/               # 回测引擎
│   ├── engine.py          # 单机/组合回测核心
│   └── optimizer.py       # 参数寻优（支持并行计算）
├── views/                  # UI 视图层
│   ├── tab_manual.py      # 手动回测看板
│   ├── tab_auto.py        # 机器参数寻优
│   ├── tab_batch.py       # 雷达全场扫描
│   ├── tab_portfolio.py   # 组合轮动实战
│   └── tab_arena.py       # 策略角斗场
├── utils/                  # 工具函数
│   ├── data_fetcher.py    # 数据获取 (AkShare + Baostock 双引擎)
...
项目默认使用 **AkShare** 作为主数据源，**Baostock** 作为备用源，无需额外配置。
...
**高阶玩法**: 千股千策 - 为组合内的不同股票配置专属策略、参数和共振板块
...
# 买入：短均线上穿长均线（金叉）
# 卖出：短均线下穿长均线（死叉）
# 可选：MACD 动能过滤（金叉时 MACD 必须>0）
```

**参数**:
- `short_window`: 短期周期 (默认 5)
- `long_window`: 长期周期 (默认 20)
- `use_macd_filter`: MACD 过滤 (默认 True)
...
- 首次拉取：自动下载全量历史数据并缓存到 `data/cache/`
- 增量更新：仅同步新增数据，极速响应
- 支持 Parquet 格式，读写高效
...
A: 系统会自动切换备用数据源。可点击侧边栏 "🔄 强制更新股票代码库" 清除缓存。
...
- 数据源：[AkShare](https://akshare.akfamily.xyz/) / [Baostock](http://baostock.com/)
